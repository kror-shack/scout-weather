...
type Props = IconHourlyHour;

interface IconHourlyMain {
  weatherCode: number;
  hour: string | "morning" | "night";
}

interface IconHourlyHour extends IconHourlyMain {
  sunrise?: string;
  sunset?: string;
}

const IconHourly = ({ weatherCode, hour, sunrise, sunset }: Props) => {
  const [selectedIcon, setSeletectedIcon] = useState<JSX.Element | null>();

  useEffect(() => {
    let isDayTime: boolean | undefined;
    if (hour === "morning") isDayTime = true;
    else if (hour === "night") isDayTime = false;
    else if (sunrise && sunset) {
      isDayTime = checkIfHourIsDay(hour, sunrise, sunset);
    }
